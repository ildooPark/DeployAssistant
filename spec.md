...
| Project Files | Col 2, tab 1 | `DataGrid` of the tracked file set with a keyword filter, plus a **폴더별 보기** (`S.GroupByFolder`) chip that swaps the grid for a `TreeView` folder hierarchy built by `ProjectFileTreeNode.Build` (Core): folder nodes show `(file count · total size)`, leaves show name + `VersionDisplay` + human-readable size + trimmed hash — no rel-path / deployed-version / time columns. The tree is rebuilt from the grid's view, so the keyword filter carries over, and refreshes on project (re)load |

...
| `TuiPromptTests` | `DeployAssistant.CLI.Tests/Engine/TuiPromptTests.cs` (or under `DeployAssistant.Tests` if the CLI test project is not added) | Pure unit: stub `Console.In` with a `StringReader` of "y\n" -> true; "n\n" -> false; "" (Esc) -> false. Note `TuiPrompt.Confirm` uses `Console.ReadKey` not `Console.ReadLine`; the test exercises the public helper by setting `Console.SetIn` and using a custom test seam, or — preferred — by extracting a `Confirm(TextReader)` overload that the unit test calls directly. The implementation plan decides; both are local. |

The CLI does not have a test harness today, so `TuiPromptTests` is the only test that motivates one. If adding a CLI test project is too large for this PR, fold the helper test into `DeployAssistant.Tests` with a `using DeployAssistant.CLI.Engine;` reference (CLI is `net472`; tests are `net8.0-windows`, so the helper would need to be in a shared assembly or the test would need to multi-target — accept that and put it in the existing project as a regular xUnit `[Fact]`, given the helper is trivial and stateless).
